...
import {
  ...
  updateCampaign,
  deleteMultiCampaign,
} from "../../../../src/redux/actions/campaign";
...
function Campaign(props) {
  ...
  const handleSave = async () => {
    ...
    props.createDraftCampaign(variables).then((value) => {
      let SplitId = value.upsertCampaign.SplitId;
      setConfirmLoading(false);
      setSuccessCampaign(true);
      setCurrentURL(
        `https://juicyfy.com?utm_source=facebook&utm_medium=ppc&split_id=${SplitId}`
      );
      props.getListCampaignDraft(authorID);
    });
  };
...
const mapStateToProps = (store) => {
  return {
    ...
    campaignData: store.campaignReducer.campaignData,
    lastAddedData: store.campaignReducer.lastAddedData,
    loading: store.campaignReducer.loading,
  };
};
